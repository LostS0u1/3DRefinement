...
if args.model==0:
 ...
 from common.model1 import *
 print("Loading model1...")
elif args.model==2:
 from common.model2 import *
...
print('Loading 3D input...')
keypoints = np.load('data/'+args.keypoints+'.npz', allow_pickle=True)
keypoints_metadata = keypoints['metadata'].item()
keypoints_symmetry = keypoints_metadata['keypoints_symmetry']
kps_left, kps_right = list(keypoints_symmetry[0]), list(keypoints_symmetry[1])
joints_left, joints_right = list(dataset.skeleton().joints_left()), list(dataset.skeleton().joints_right())
keypoints = keypoints['positions_3d'].item()

for subject in dataset.subjects():
  if 'S7' not in subject:
    assert subject in keypoints, 'Subject {} is missing from the 2D detections dataset'.format(subject)
    for action in dataset[subject].keys():
      if "Walking" in action:
...
if not args.disable_optimizations and not args.dense and args.stride == 1  :
    ...
    print('Loading single frame training model')
...
if not args.evaluate:
    ...
    while epoch < args.epochs:
        ...
        if epoch % args.checkpoint_frequency == 0:
            if args.ownharddisk :
             chk_path = os.path.join('/media/wanghao/TOSHIBA EXT/Evaluation_result/', ('epoch_{}_{}_lr{}_lrd{}.bin'.format(epoch,time.asctime( time.localtime(time.time()) ),lr,lr_decay) ))
            else:
             chk_path = os.path.join(args.checkpoint, ('epoch_{}_{}.bin'.format(epoch,time.asctime( time.localtime(time.time()) )) ))
